...
| `spread` | $\Delta = \min\left(1,\ \frac{\sum_{i=1}^{n-1}\lvert d_i - \bar{d}\rvert}{(n-1)\bar{d}}\right)$, where $d_i$ is the Euclidean gap between neighbours sorted by the first objective (2 objectives only; unlike Deb's $\Delta$, no extreme-point terms) | Uniformity of distribution along the front | **Near 0**: evenly spaced front. **Near 1**: uneven gaps, clustered solutions. | `1.0` when fewer than 2 solutions are on the front; `0.0` when all front solutions coincide; absent for 3+ objectives. |
...
| `ensemble/top_k_concentration` | $C_k = \dfrac{\sum_{i \in \text{top-}k} f_i}{\sum_i f_i}$, range $[0, 1]$, where top-$k$ is the expert set also used for turnover (best `elite_size`, else top `top_k_percent`, ranked in the run's direction, feasibility first) | Fraction of total fitness held by the top-$k$% | **High (→ 1)**: the top fraction monopolises fitness, likely indicating star topology or runaway selection. **Low (→ 0)**: fitness is dispersed across the population. | Returns `0.0` when total fitness is zero — when no individual has nonzero fitness, no individual concentrates it. |
