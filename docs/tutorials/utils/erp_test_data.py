...
def create_test_population_with_protocols(
    n: int, config: dict | None = None, seed: int = 42, rng: Random | None = None
) -> Population:
    ...
    for i in range(n):
        ...
        individual = Individual(
            genome=genome, fitness=Fitness(values=(rng.uniform(*fitness_range),)), protocol=protocol
        )
...
def _create_diverse_protocol(index: int, total: int, rng: Random) -> ReproductionProtocol:
    ...
    if strategy_index == 0:
        ...
        matchability = MatchabilityFunction(
            type="distance_threshold", params={"max_distance": rng.uniform(2.0, 8.0)}
        )
    elif strategy_index == 1:
        # Fitness threshold, accept all
        intent = ReproductionIntentPolicy(
            type="fitness_threshold", params={"threshold": rng.uniform(30.0, 70.0)}
        )
        ...
        intent = ReproductionIntentPolicy(type="always_willing")
        matchability = MatchabilityFunction(
            type="fitness_proportional", params={"selectivity": rng.uniform(0.3, 0.9)}
        )
    else:
        # Stochastic intent, diverse preference
        intent = ReproductionIntentPolicy(
            type="stochastic", params={"probability": rng.uniform(0.5, 1.0)}
        )
        matchability = MatchabilityFunction(
            type="diversity_preference", params={"min_distance": rng.uniform(1.0, 3.0)}
        )
...
def _create_selective_protocol(rng: Random) -> ReproductionProtocol:
    ...
    return ReproductionProtocol(
        intent=ReproductionIntentPolicy(type="fitness_threshold", params={"threshold": 70.0}),
        matchability=MatchabilityFunction(type="distance_threshold", params={"max_distance": 2.0}),
...
def _create_default_protocol() -> ReproductionProtocol:
    ...
    return ReproductionProtocol(
        intent=ReproductionIntentPolicy(type="always_willing"),
        matchability=MatchabilityFunction(type="distance_threshold", params={"max_distance": 5.0}),
        crossover=CrossoverProtocolSpec(type=CrossoverType.UNIFORM),
    )
...
def create_diverse_protocol_set() -> list[ReproductionProtocol]:
    ...
    protocols.append(
        ReproductionProtocol(
            ...
            matchability=MatchabilityFunction(
                type="distance_threshold", params={"max_distance": 3.0}
            ),
    ...
    protocols.append(
        ReproductionProtocol(
            intent=ReproductionIntentPolicy(type="always_willing"),
            matchability=MatchabilityFunction(
                type="fitness_proportional", params={"selectivity": 0.8}
            ),
    ...
    protocols.append(
        ReproductionProtocol(
            intent=ReproductionIntentPolicy(type="always_willing"),
            matchability=MatchabilityFunction(
                type="diversity_preference", params={"min_distance": 2.0}
            ),
...
def create_sexual_selection_population(
    ...
    female_protocol = ReproductionProtocol(
        intent=ReproductionIntentPolicy(type="always_willing"),
        matchability=MatchabilityFunction(type="fitness_based", params={"min_fitness": 60.0}),
        crossover=CrossoverProtocolSpec(type=CrossoverType.UNIFORM),
    )
    ...
    for _ in range(n_males):
        genome = VectorGenome.random(dimensions, bounds, rng)
        individual = Individual(
            genome=genome, fitness=Fitness(values=(rng.uniform(0, 100),)), protocol=male_protocol
        )
        individuals.append(individual)
    ...
    for _ in range(n_females):
        genome = VectorGenome.random(dimensions, bounds, rng)
        individual = Individual(
            genome=genome, fitness=Fitness(values=(rng.uniform(0, 100),)), protocol=female_protocol
        )
        individuals.append(individual)
