...
def get_benchmark(name: str, dimensions: int = 10) -> BenchmarkFunction:
    ...
    if dimensions != benchmark.dimensions:
        optimal_pos = (
            np.zeros(dimensions)
            if name_lower in ("sphere", "rastrigin", "ackley")
...
def plot_population_diversity(
    ...
    if method == "pca":
        ...
        reducer = TSNE(
            n_components=2, random_state=42, perplexity=min(30, len(population_genomes) - 1)
        )
        coords = reducer.fit_transform(population_genomes)

    if fitness_values is not None:
        scatter = ax.scatter(
            coords[:, 0], coords[:, 1], c=fitness_values, cmap="viridis", alpha=0.7
        )
...
def plot_pareto_3d_interactive(
    ...
    obj_names = (
        front.objective_names if len(front.objective_names) >= 3 else ["Obj 0", "Obj 1", "Obj 2"]
    )
...
def plot_species_phylogeny(
    ...
    for i, sid in enumerate(species_ids):
        ...
        death = history.species_extinctions.get(
            sid, max(history.generations) if history.generations else 0
        )
...
def plot_causal_graph_comparison(
    ...
    def draw_graph(
        ax: plt.Axes, adj: np.ndarray, graph_title: str, edge_colors: dict | None = None
    ) -> None:
        G = nx.DiGraph()
        ...
        if edge_colors:
            for edge in G.edges():
                ...
                nx.draw_networkx_edges(
                    G, pos, ax=ax, edgelist=[edge], edge_color=color, arrows=True
                )
        else:
...
def plot_protocol_evolution(
    history: ERPHistory, param_name: str, ax: plt.Axes | None = None, show_std: bool = True
) -> plt.Figure:
...
def plot_mating_network(
    ...
    for i in range(n_individuals):
        fitness = (
            population.individuals[i].fitness.values[0]
    ...
    nx.draw_networkx_nodes(
        G, pos, node_color=fitness_values, node_size=degrees, cmap="viridis", alpha=0.7, ax=ax
    )
    ...
    sm = plt.cm.ScalarMappable(
        cmap="viridis", norm=plt.Normalize(vmin=min(fitness_values), vmax=max(fitness_values))
    )
    sm.set_array([])
            ...
            try:
                ...
                if hasattr(ind_i.genome, "genes") and hasattr(ind_j.genome, "genes"):
                    ...
                    if hasattr(matchability_function, "parameters"):
                        threshold = matchability_function.parameters.get(
...
def plot_recovery_events(
    history: ERPHistory, recovery_log: list[dict], ax: plt.Axes | None = None
) -> plt.Figure:
