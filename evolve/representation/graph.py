...
class GraphGenome:
    ...
    def get_connection_by_nodes(self, from_node: int, to_node: int) -> ConnectionGene | None:
    ...
    def hidden_nodes(self) -> frozenset[NodeGene]:
        ...
        return frozenset(n for n in self.nodes if n.node_type == "hidden")

    def input_nodes(self) -> frozenset[NodeGene]:
        """Get input nodes."""
        return frozenset(n for n in self.nodes if n.node_type == "input")

    def output_nodes(self) -> frozenset[NodeGene]:
        """Get output nodes."""
        return frozenset(n for n in self.nodes if n.node_type == "output")
    ...
    def add_node(
        ...
        new_connections = self.connections - {split_connection} | {
            disabled_conn,
            new_conn1,
            new_conn2,
        }
    ...
    def with_connection_weight(self, innovation: int, weight: float) -> GraphGenome:
