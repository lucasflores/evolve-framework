...
class RecurrentNumpyNetwork:
    ...
    def __call__(self, x: np.ndarray) -> np.ndarray:
        ...
        self.hidden_state = self.hidden_activation(h_input + h_recurrent + self.hidden_bias)

        # Compute output
        output = self.output_activation(self.hidden_state @ self.output_weights + self.output_bias)
...
class NEATNetwork:
    ...
    def __call__(self, x: np.ndarray) -> np.ndarray:
        ...
        # Collect outputs
        outputs = np.array([node_values.get(output_id, 0.0) for output_id in self.output_ids])
