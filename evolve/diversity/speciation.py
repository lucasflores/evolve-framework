...
def neat_distance(
    ...
    if matching:
        ...
        weight_diff = sum(abs(a_weights[i] - b_weights[i]) for i in matching) / len(matching)
    else:
        weight_diff = 0.0
...
class KMeansSpeciator(Generic[G]):
    ...
    def speciate(
        ...
        if len(population) < self.n_species:
            ...
            return [
                Species(id=i, representative=ind, members=[ind]) for i, ind in enumerate(population)
            ]
            ...
            for i, members in enumerate(assignments):
                if members:
                    ...
                    best_member = min(
                        members,
                        key=lambda m: sum(
                            self.distance_fn(m.genome, other.genome) for other in members
                        ),
                    )
