...
from evolve.diversity.islands.migration import MigrationController, BestMigration
...
class IslandEvolutionEngine(Generic[G]):
    ...
    def _evaluate_all_islands(self) -> None:
        ...
        for island in self._islands:
            ...
            unevaluated = [
                ind for ind in island.population
        ...
        for island in self._islands:
            ...
            if island_fitness:
                stats["islands"][island.id] = {
                    "size": island.size,
                    "best_fitness": min(island_fitness) if self.config.minimize else max(island_fitness),
