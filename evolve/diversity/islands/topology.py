...
def ring_topology(n_islands: int) -> dict[int, list[int]]:
    ...
    return {i: [(i - 1) % n_islands, (i + 1) % n_islands] for i in range(n_islands)}
...
def fully_connected_topology(n_islands: int) -> dict[int, list[int]]:
    ...
    """
    if n_islands < 2:
        raise ValueError(f"Fully connected topology requires at least 2 islands, got {n_islands}")

    return {i: [j for j in range(n_islands) if j != i] for i in range(n_islands)}
...
def hypercube_topology(n_islands: int) -> dict[int, list[int]]:
    ...
    """
    if n_islands < 2:
        raise ValueError(f"Hypercube topology requires at least 2 islands, got {n_islands}")

    if n_islands & (n_islands - 1) != 0:
        raise ValueError(f"Hypercube topology requires power of 2 islands, got {n_islands}")
...
def ladder_topology(n_islands: int) -> dict[int, list[int]]:
    ...
    """
    if n_islands < 2:
        raise ValueError(f"Ladder topology requires at least 2 islands, got {n_islands}")
...
def star_topology(n_islands: int) -> dict[int, list[int]]:
    ...
    """
    if n_islands < 2:
        raise ValueError(f"Star topology requires at least 2 islands, got {n_islands}")
