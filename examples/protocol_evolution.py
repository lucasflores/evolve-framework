    ...
    for _ in range(size):
        ...
        protocol = ReproductionProtocol(
            ...
            matchability=MatchabilityFunction(
                ...
                params={"min_fitness": threshold}
            ),
            crossover=CrossoverProtocolSpec(
                type=crossover_type,
                params={"swap_probability": swap_prob} if crossover_type == CrossoverType.UNIFORM else {}
            )
        )
...
class ProtocolTracker:
    ...
    def print_summary(self):
        ...
        print(f"  Generation {len(self.generations)-1}: mean={final_mean:.3f}, std={final_std:.3f}")
        ...
        print(f"  Generation {len(self.generations)-1}: mean={final_swap_mean:.3f}, std={final_swap_std:.3f}")
...
def main():
    ...
    print(f"  • Thresholds: 0.2 - 0.9 (uniform random)")
    print(f"  • Crossover types: uniform, single-point, two-point")
    ...
    result = erp_engine.run(
        initial_population=population,
        callbacks=[history, tracking_callback]
    )
