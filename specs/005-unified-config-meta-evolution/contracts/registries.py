...
def _register_builtin_operators(registry: OperatorRegistry) -> None:
    ...
    registry.register(
        "crossover", "uniform", UniformCrossover, compatible_genomes={"vector", "sequence"}
    )
    registry.register(
        "crossover", "single_point", SinglePointCrossover, compatible_genomes={"vector", "sequence"}
    )
    registry.register(
        "crossover", "two_point", TwoPointCrossover, compatible_genomes={"vector", "sequence"}
    )
...
class GenomeRegistry:
    ...
    def create(self, name: str, rng: Random, **params: Any) -> Any:
        ...
        if name not in self._factories:
            ...
            raise KeyError(f"Genome type '{name}' not registered. Available: {available}")
