...
class StoppingConfig:
    ...
    def is_empty(self) -> bool:
        ...
        return all(
            ...
            for f in ("max_generations", "fitness_threshold", 
                     "stagnation_generations", "time_limit_seconds")
        )
...
class UnifiedConfig:
    ...
    def _multiobjective_to_dict(self) -> dict[str, Any] | None:
        ...
        return {
            ...
            "reference_point": list(self.multiobjective.reference_point) 
                if self.multiobjective.reference_point else None,
            "constraints": [
    ...
    def from_dict(cls, data: dict[str, Any]) -> Self:
        ...
        if data.get("multiobjective"):
            ...
            objectives = tuple(
                ObjectiveSpec(**o) for o in mo_data.get("objectives", [])
            )
            constraints = tuple(
                ConstraintSpec(**c) for c in mo_data.get("constraints", [])
            )
            multiobjective = MultiObjectiveConfig(
                objectives=objectives,
                reference_point=tuple(mo_data["reference_point"]) 
                    if mo_data.get("reference_point") else None,
