...
class TestNSGA2Selection:
    ...
    def test_selection_prefers_lower_rank(self, rng):
        ...
        for i in range(5):
            genome = VectorGenome(
                genes=np.array([0.1 * i, 0.1 * (4 - i)]), bounds=(np.zeros(2), np.ones(2))
            )
        ...
        for i in range(5):
            genome = VectorGenome(
                genes=np.array([0.5 + 0.1 * i, 0.5 - 0.1 * i]), bounds=(np.zeros(2), np.ones(2))
            )
    ...
    def test_crowded_tournament_selection(self, rng):
        ...
        population = []
        for i in range(10):
            genome = VectorGenome(genes=np.array([0.1 * i]), bounds=(np.zeros(1), np.ones(1)))
...
class TestNSGA2Integration:
    ...
    def test_zdt1_pareto_front_approximation(self, rng, zdt1_bounds):
        ...
        front_points = np.array([-population[i].fitness.objectives for i in front_indices])
