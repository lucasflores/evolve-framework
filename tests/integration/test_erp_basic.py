...
class TestAsymmetricMatchability:
    ...
    def test_symmetric_acceptance_allows_reproduction(self) -> None:
        ...
        a_accepts_b, _ = safe_evaluate_matchability(
            ind_a.protocol.matchability, ctx_a_to_b, rng
        )
        b_accepts_a, _ = safe_evaluate_matchability(
            ind_b.protocol.matchability, ctx_b_to_a, rng
        )
...
class TestIntentBeforeMatchability:
    ...
    def test_willing_proceeds_to_matchability(self) -> None:
        ...
        # Check intent
        intent_a, _ = safe_evaluate_intent(
            ind_a.protocol.intent, create_intent_context(ind_a), rng
        )
        intent_b, _ = safe_evaluate_intent(
            ind_b.protocol.intent, create_intent_context(ind_b), rng
        )
        ...
        a_accepts, _ = safe_evaluate_matchability(
            ind_a.protocol.matchability, ctx_a_to_b, rng
        )
        b_accepts, _ = safe_evaluate_matchability(
            ind_b.protocol.matchability, ctx_b_to_a, rng
        )
...
class TestDormantLogicActivation:
    ...
    def test_junk_data_mutation_creates_dormant_params(self) -> None:
        ...
        assert len(mutated.junk_data) > len(protocol.junk_data), \
            "Mutation should add junk data"
    ...
    def test_junk_data_preserved_through_evolution(self) -> None:
        ...
        assert restored.junk_data == protocol.junk_data, \
            "Junk data should survive serialization"
