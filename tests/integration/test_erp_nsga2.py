...
def create_pareto_population(
    ...
    for i in range(size // 2):
        ...
        obj2 = 1.0 - obj1
        individuals.append(
            create_multiobjective_individual(rng, [obj1, obj2])
        )
    ...
    for i in range(size - size // 2):
        ...
        individuals.append(
            create_multiobjective_individual(rng, [obj1, max(0, obj2)])
        )
...
class TestSelectionAuthority:
    ...
    def test_erp_does_not_override_pareto_ranking(self) -> None:
        ...
        mo_fitnesses = [
            MultiObjectiveFitness(ind.fitness.values) 
    ...
    def test_dominated_individual_can_still_mate(self) -> None:
        ...
        accepts, _ = safe_evaluate_matchability(
            dominated.protocol.matchability, mate_ctx, rng
        )
...
class TestERPWithNSGAII:
    ...
    def test_matchability_can_access_crowding_distance(self) -> None:
        ...
        result, success = safe_evaluate_matchability(
            ind_a.protocol.matchability, ctx, rng
        )
    ...
    def test_nsga2_workflow_with_erp_mating(self) -> None:
        ...
        for i in range(0, len(parents) - 1, 2):
            ...
            intent1, _ = safe_evaluate_intent(
                parent1.protocol.intent,
                create_intent_context(parent1),
                rng
            )
            intent2, _ = safe_evaluate_intent(
                parent2.protocol.intent,
                create_intent_context(parent2),
                rng
            )
            ...
            match1, _ = safe_evaluate_matchability(
                parent1.protocol.matchability, ctx1, rng
            )
            match2, _ = safe_evaluate_matchability(
                parent2.protocol.matchability, ctx2, rng
            )
