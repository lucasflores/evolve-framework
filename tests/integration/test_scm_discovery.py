...
class TestSCMERPIntegration:
    ...
    def test_scm_matchability_with_similarity_threshold(self, basic_config, rng, counter):
    ...
    def test_scm_matchability_with_distance_threshold(self, basic_config, rng, counter):
    ...
    def test_structural_weight_affects_matchability_outcome(self, basic_config, rng, counter):
        """
        ...
        if dist_with_struct > dist_seq_only:
            ...
            result_struct = evaluator.evaluate(
                context_struct, {"max_distance": threshold}, rng, counter
            )

            # The key assertion: structural weight changes the outcome
            assert result_seq != result_struct or dist_with_struct == dist_seq_only

    def test_matchability_evaluation_completes_without_error(self, basic_config, rng, counter):
        ...
        genomes = [SCMGenome.random(basic_config, length=20, rng=Random(i)) for i in range(10)]
        decoder = SCMDecoder(basic_config)
...
class TestEndToEndSCMDiscovery:
    ...
    def test_scm_genome_evaluator_workflow(self, synthetic_3var_data, rng):  # noqa: ARG002
        ...
        evaluator = SCMEvaluator(
            data=data, variable_names=var_names, config=fitness_config, decoder=decoder
        )

        # Create a population of genomes
        population_size = 20
        genomes = [
            SCMGenome.random(config, length=50, rng=Random(i)) for i in range(population_size)
        ]
    ...
    def test_population_fitness_variance(self, synthetic_3var_data, rng):  # noqa: ARG002
        ...
        genomes = [SCMGenome.random(config, length=50, rng=Random(i * 100)) for i in range(50)]
...
class TestPopulationEvolution:
    ...
    def test_large_population_creation(self, basic_config, rng):  # noqa: ARG002
        ...
        genomes = [
            SCMGenome.random(basic_config, length=50, rng=Random(i)) for i in range(population_size)
        ]
    ...
    def test_simulated_generations(self, basic_config, rng):  # noqa: ARG002
        ...
        for gen in range(n_generations):
            ...
            valid_fitness_values = [fv for _, fv in genome_fitness_pairs if fv > float("-inf")]
            ...
            sorted_pairs = sorted(genome_fitness_pairs, key=lambda x: x[1], reverse=True)
...
class TestDecodingPerformance:
    ...
    def test_batch_decode_performance(self, basic_config, rng):  # noqa: ARG002
        ...
        genomes = [SCMGenome.random(basic_config, length=100, rng=Random(i)) for i in range(100)]
