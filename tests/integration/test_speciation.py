...
class TestThresholdSpeciator:
    ...
    def test_speciation_groups_similar(self):
        ...
        # Create two clusters
        cluster1 = [
            create_test_individual(f"c1_{i}", np.array([0.0, float(i) * 0.1])) for i in range(5)
        ]
        cluster2 = [
            create_test_individual(f"c2_{i}", np.array([10.0, float(i) * 0.1])) for i in range(5)
        ]
    ...
    def test_speciation_preserves_existing(self):
        ...
        population = [create_test_individual("new", np.array([0.1]))]
    ...
    def test_speciation_removes_empty(self):
        ...
        population = [create_test_individual("new", np.array([0.0]))]
...
class TestKMeansSpeciator:
    ...
    def test_kmeans_creates_k_species(self):
        ...
        population = [create_test_individual(f"ind_{i}", np.random.randn(5)) for i in range(20)]
    ...
    def test_crowding_selects_winner(self):
        ...
        offspring = [
            create_test_individual(
                "c1", np.array([0.1]), fitness_value=10.0
            ),  # Better, similar to p1
            create_test_individual(
                "c2", np.array([10.1]), fitness_value=3.0
            ),  # Worse, similar to p2
        ]

        survivors = deterministic_crowding_pairing(parents, offspring, euclidean_distance)
