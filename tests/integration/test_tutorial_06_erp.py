...
TUTORIAL_PATH = Path(__file__).parent.parent.parent / "docs" / "tutorials" / "06_evolvable_reproduction_protocols.ipynb"
...
class TestTutorial06ERP:
    ...
    def test_notebook_has_visualizations(self):
        ...
        assert len(viz_cells) >= 5, f"Expected at least 5 visualization cells, found {len(viz_cells)}"
...
class TestTutorialContent:
    ...
    def test_sexual_selection_example(self):
        ...
        assert "choosy" in content.lower() or "selective" in content.lower(), "Choosy mating not demonstrated"
...
def test_tutorial_performance_benchmark():
    ...
    print(f"\n{'=' * 60}")
    print(f"Tutorial 6 Performance Benchmark")
    print(f"{'=' * 60}")
