            ...
            try:
                ...
                if generation % 100 == 0:
                    ...
                    safe_evaluate_matchability(
                        adversarial.matchability, mate_ctx, rng
                    )
...
class TestPerformanceBenchmarks:
    ...
    def test_full_mating_cycle_overhead(self) -> None:
        ...
        genomes = [
            np.array([rng.random() for _ in range(genome_size)])
            for _ in range(100)
        ]
        ...
        for i in range(n_iterations):
            ...
            if intent_ok:
                counter.reset()
                match_ok, _ = safe_evaluate_matchability(
                    protocol.matchability, mate_ctx, rng
                )
                
                if match_ok:
                    # Execute crossover
                    (child1, child2), _ = safe_execute_crossover(
                        protocol.crossover, p1, p2, rng
                    )
