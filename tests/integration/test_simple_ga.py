...
from evolve.evaluation.reference.functions import rastrigin, sphere
...
class TestGAComponents:
    ...
    def test_tournament_selection_pressure(self):
        ...
        n = 100
        individuals = [
            Individual(genome=VectorGenome(genes=np.array([float(i)]))).with_fitness(
                Fitness.scalar(float(i))
            )  # fitness = index
            for i in range(n)
        ]
        pop = Population(individuals)
